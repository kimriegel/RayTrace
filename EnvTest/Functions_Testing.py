...
def Grid(radius,A,B,C,D,xmin,ymin,zmin,xmax,ymax,zmax,receiverarray, arraysize,step):
    """
    We used to think grid was unused.
    We were wrong. It is only used in complex receivers.
    After that it's totally unused though.

    A,B,C, and D are the different receivers
...
def SPHERECHECK(Sc,Sr2,F,veci):
    ...
    Glassner Page 42:
    We know /that/ it hit something. But where

    If it originates outside the sphere 
    ...
    #OC[1]=Sc[1]-veci[1,0]
    #OC[2]=Sc[2]-veci[2,0]
    #L2OC=np.dot(OC[:,0],OC[:,0])
    #L2OC=np.dot(OC,OC)

    #print('Old: ', L2OC)
    #my_l2oc = dot(OC2,OC2)
    L2OC = dot(OC2,OC2) # My version 
    #print(L2OC)
    #print('Nuevo: ', my_l2oc[0])
    #print(OC2[:,0]*OC2[:,0])
    ...
    #print(t2hc)
    """ 
    For each ray, where the condition is met dx = Huge, else dx is that other thing.
    dx is now an array with values for all rays at once.
    """
    #print(L2OC)
    #print(L2OC == Sr2)
    #print(tca)
    #print(t2hc)
    #print('Sr2: ',Sr2)
    #print('L2OC: ',L2OC[607:610])
    #print('tca: ',tca[607:610])
    #print('t2hc: ',t2hc[607:610])

    """
    Initialize the array. The else condition has been moved to the default.
    If the conditions are not met then the else condition will be returned.
    """
    # Hey so, that's literally how the if-condition works normally, but this works 
    # faster than the normal version, does everything at once without bugs, and took a day of planning
    dx = tca - np.sqrt(np.abs(t2hc))
    #print(dx[605:610])
    #print(t2hc[605:610])
    dx = np.where((L2OC == Sr2), HUGE, dx )
    dx = np.where((tca < 0.0),   HUGE, dx )
    dx = np.where((t2hc < 0.0),  HUGE, dx )
...
def dot(A,B):
    """
    It's just a dot product, but the gimmick is that
    this works on whatever size array you feed it, as 
    opposed to only one dot product at a time.

    Edit: This array is for two 2d arrays. I have one 
    for a 2d array and a 1d array now.
    """    
    # Scrap paper included to remind me of my hubris
    # Memento Mori 
    i  =  A[0,:] * B[0,:] 
    j  =  A[1,:] * B[1,:] 
    k  =  A[2,:] * B[2,:]
    #print('My function: ',i[0],j[0],k[0])
    product = i + j + k 
    return product
    #product = i + j + k 
    #print(product[0])
    ...
    #print('Old: ',OC)
    #print('New: ',OC2[:,0])
    #""" Same Behavior In Functions """
    #print('Nuevo: ',np.dot(OC2[:,0],OC2[:,0]))
    #print('Viejo: ',np.dot(OC,OC))
    #""" The Algebra """
    #print('Algebra: ',OC2[0,0] * OC2[0,0], '+', OC2[1,0] * OC2[1,0], '+', OC2[2,0] * OC2[2,0]  )
        #Solo
    #product = OC2[0,0] * OC2[0,0] + OC2[1,0] * OC2[1,0] + OC2[2,0] * OC2[2,0]
    #print('Respuesta: ', product)
        #Todo
    #product = OC2[0,:] * OC2[0,:] + OC2[1,:] * OC2[1,:] + OC2[2,:] * OC2[2,:]
    #print('Primera: ', product[0])

def dot2_1(A,B):
    """
    Takes a dot product of each row of a 2d array
    in relation to a 1d array. 

    If this isn't working remember: A = 2d, B = 1d
    """    
