...
# What it does not do
"""
    Have a ray that hits a receiver, they all miss
...
for I in range(3):
    ...
    """
            constant(c) can be made in a single array,  flux(f) is more complex
    """
    #m=airabsorb[:]      #c
    #lamb=PF.soundspeed/ frecuencias[:,0]   #c
    phasefinal=phaseinitial[:]-(twopih)/lamb[:]   #f
    phaseinitial[:]=phasefinal % twopi          #f
    print(phaseinitial.shape)
            # Muda Da
    ...
    #            tempreceiver=HUGE
       ...
       #
        ...
        # Where ray hits receiver, return properties, else 0
        #veci[0,:] = R.position #bug fixing
        #print(R.position)
        hits = hit_cz(R.position,veci)
        if np.any(hits==1):
            #print(np.sum(ampinitial*hits[:,None],axis=0).shape )       #Sum of all rays that hit, individually by frequency
            hitamp = np.where(         hits[:,None],
                                        ampinitial*hits[:,None],
                                        0)
            #print(tempamp)
            #np.sum(ampinitial*hits[:,None],axis=0)

            #print(R.magnitude)
            hitphase = np.where(         hits[:,None],
                                        phaseinitial*hits[:,None],
                                        0)
            RPS.Receiver.on_Hit(R,hitamp,hitphase)
            #print(tempamp.shape)
            #print(R.direction)
            #print(tempdir==False)
            #print(np.product(hit_cz(R.position,veci)))
...
for R in RPS.Receiver.rList:
    ...
    #print(isinstance(R.magnitude,int))
    if isinstance(R.magnitude,int):
        """
        If no rays hit a receiver the program crashes otherwise
        """
...
OPFile=open(PF.OUTPUTFILE,"a")      #redefining to print  TimeHeader without deleting Header
