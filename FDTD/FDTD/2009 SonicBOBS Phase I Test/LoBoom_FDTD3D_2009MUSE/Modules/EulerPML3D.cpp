 ...
 *  LoBoomFDTD3D_MultiBldg program.  The grid points inside the PML and points 
 ...
 *    Acute concave corners that are nearing rigid boundaries on both sides in
 *  a coordinate direction are treated separately.  At these points, the
 *  pressure and velocity components in that coordinate direction are set to
 *  zero since the centered-difference scheme uses the variables at the two 
 *  adjacent points with the same values duplicated from the center by the rigid
 *  boundary condition.   
 *    
 *******************************************************************************/     
      ...
      for (k=1; k<K_MAX+1; k++) {
        if (U[i][j][k].InOrOut==0) {
          ...
          if (i==1) {           // PML region @ domain boundary
            ...
            if (U[i-1][j][k].InOrOut==1) {      // Neighboring a rigid boundary in (-)ve x-direction
              if (U[i+1][j][k].InOrOut==1) {    // Also neighboring a rigid boundary in (+)ve x-direction (acute concave corner in x-direction)
                dU[i][j][k].px = 0;
                dU[i][j][k].u = CFLo2z0*(U[i+1][j][k].py-U[i-1][j][k].py+U[i+1][j][k].pz-U[i-1][j][k].pz);
              }
              else {
            ...
            else if (U[i+1][j][k].InOrOut==1) {   // Neighboring a rigid boundary in (+)ve x-direction
          ...
          if (j==1) {           // PML region @ domain boundary
            ...
            if (U[i][j-1][k].InOrOut==1) {      // Neighboring a rigid boundary in (-)ve y-direction
              if (U[i][j+1][k].InOrOut==1) {    // Also neighboring a rigid boundary in (+)ve y-direction (acute concave corner in y-direction)
                dU[i][j][k].py = 0;
                dU[i][j][k].v = CFLo2z0*(U[i][j+1][k].px-U[i][j-1][k].px+U[i][j+1][k].pz-U[i][j-1][k].pz);
              }
              else {
            ...
            else if (U[i][j+1][k].InOrOut==1) {   // Neighboring a rigid boundary in (+)ve y-direction
          ...
          if (k==1) {           // Euler region @ domain boundary
            ...
            if (U[i][j][k-1].InOrOut==1) {        // Neighboring a rigid boundary in (-)ve z-direction
