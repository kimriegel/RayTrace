 ...
 *  sonic boom for LoBoomFDTD3D_SingleBldg program.  Both the incident wave and 
...
void InitializeMatrix(Grid3D &U, Array &sigma, Array &boom_shape)
{
  ...
  // Fill in the domain with incident & reflected booms with appropriate incoming angles
  a_inc = -cos(phi)*cos(theta);
  b_inc = -sin(phi)*cos(theta);
  c_inc = -sin(theta);
  d_inc = -a_inc*i_intersect;
  a_ref = -cos(phi)*cos(theta);
  b_ref = -sin(phi)*cos(theta);
  c_ref = sin(theta);
    ...
    for (j=1; j<J_MAX+1; j++) {
      for (i=1; i<I_MAX+1; i++) {
        if (i>=wall_loc_i1 && i<=wall_loc_i2 && j>=wall_loc_j1 && j<=wall_loc_j2 && k>=wall_loc_k1 && k<=wall_loc_k2)
          ...
          // Time for the boom wavefront to get to a grid point, measured in time index
