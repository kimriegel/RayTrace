 ...
 *    This version of the code uses a simplified building geometry, which is 
 *  made by removing the roof overhang from the original test house used in 2006
 *  NASA flight test.  The result of this simplified model can be compared to 
 *  the original model result for the purpose of determining the importance of 
 *  including this geometry detail in FDTD simulation. 
 *                 
 *******************************************************************************/
...
int main(int args, char *argv[])
{
  ...
  for ( j = 0; j < z_indmax; j++ ) {
    for ( i = 0; i < x_indmax; i++ ) {
      ...
      if ( x_pos[i] >= x0 && x_pos[i] <= x0+8.8458 && z_pos[j] <= tan14*x_pos[i]+z2 && z_pos[j] <= -tan14*x_pos[i]+z4 )
        p[i][j] = 100;
    }
  }
  ...
  for ( n = 0; n < t_indmax; n++ ) {
    ...
    for ( j = 0; j < z_indmax; j++ ) {
      for ( i = 0; i < x_indmax; i++ ) {
        if ( i == (int)ceil(x0/dx) && z_pos[j] < tan14*x_pos[i]+z2 )
          p[i][j] = p[i-1][j];
        if ( i == (int)floor((x0+8.8458)/dx) && z_pos[j] < -tan14*x_pos[i]+z4 )
          p[i][j] = p[i+1][j];
        if ( j == (int)floor((tan14*x_pos[i]+z2)/dx) && x_pos[i] > x0 && x_pos[i] < x0+4.4229 )
          p[i][j] = p[i][j+1];
        if ( j == (int)floor((-tan14*x_pos[i]+z4)/dx) && x_pos[i] > x0+4.4229 && x_pos[i] < x0+8.846 )
          p[i][j] = p[i][j+1];
      }
    }
