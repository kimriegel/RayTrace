%% Analy_Diffr_HalfPlane_LoBoom_PtRcvr.m
%
%   written by Sang-Ik Terry Cho
%   last modified on 1 / 18 / 2012
%
...
%   realistic input.
%       The example case presented in this program models a Sommerfeld-type
%   rigid half plane, which corresponds to dp/dn = 0 on the boundary.  The 
%   half plane lies on the (+)ve x-axis starting at the origin and the 
%   radial angle is defined going counterclockwise from the (+)ve x-axis.
...
load('input_boom.txt');
input_wave = input_boom(29900:60000);
fs_input = 25600;
theta0_deg = 30;
...
%% Propagation
for n = 1:N
    P_inc(n) = Heaviside(pi-Theta1)*Heaviside(tt(n)+R*cos(Theta1)/c0)*Signature(tt(n)+R*cos(Theta1)/c0,fs_input,input_wave_normalized);
    P_ref(n) = Heaviside(pi-Theta2)*Heaviside(tt(n)+R*cos(Theta2)/c0)*Signature(tt(n)+R*cos(Theta2)/c0,fs_input,input_wave_normalized);

    if tt(n)-R/c0 >= 0  % replacing Heaviside(tt(n)-R/c)
        Integrand1 = @(S)Signature(tt(n)-R/c0-S,fs_input,input_wave_normalized)./(sqrt(S).*(S+R/c0*(1+cos(Theta1))));
        Integrand2 = @(S)Signature(tt(n)-R/c0-S,fs_input,input_wave_normalized)./(sqrt(S).*(S+R/c0*(1+cos(Theta2))));
        P_dif1(n) = -sign(pi-Theta1)*sqrt(R/c0*(1+cos(Theta1)))/(2*pi)*Heaviside(tt(n)-R/c)*quadgk(Integrand1,0,tt(n)-R/c0,'RelTol',reltol);
        P_dif2(n) = -sign(pi-Theta2)*sqrt(R/c0*(1+cos(Theta2)))/(2*pi)*Heaviside(tt(n)-R/c)*quadgk(Integrand2,0,tt(n)-R/c0,'RelTol',reltol);
    end
end
